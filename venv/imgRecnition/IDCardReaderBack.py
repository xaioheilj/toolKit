...
class ID_OCR:
    ...
    def white_excel(self, file_name):
        ...
        headings = ['签发机关',"签发日期", "失效日期"]  # 设置表头数据
    ...
    def post_api(self, img_url):
        ...
        params = {"id_card_side": "back", "image": img}  # 写入body参数,front代表正面
        ...
        if response:
            print(response.json())
            try:
                issAuth = response.json().get("words_result").get("签发机关").get("words")
                getDate = response.json().get("words_result").get("签发日期").get("words")
                loseDate = response.json().get("words_result").get("失效日期").get("words")
...
if __name__ == '__main__':
    ...
    filename1 = timestr
    p1.run(file_url="D://Picture/IDCard_back/", filename='/'+filename1, ext="")